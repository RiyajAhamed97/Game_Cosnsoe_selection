...
function comment()
{
    return json_decode(file_get_contents(__DIR__ . '/comment.json'), true);
}
...
function putJson_cm($users)
{
    file_put_contents(__DIR__ . '/comment.json', json_encode($users_cm, JSON_PRETTY_PRINT));
}
function validateUser($user, &$errors)
{
    ...
    if ($user==1) {
        $isValid = false;
        $errors['email'] = 'Its already taken';
    }
    // End Of validation
